...
def check_vector(msg,x,f,df):
    y=f(forward.seed(x))
    dy=sparse.standard.diagonalMatrix(len1d(x),lambda:df(x))
    assert_almost_equal(y.value,f(x))
...
def check_vector_scalar(msg,x,v,f,df):
    ...
    assert y.dvalue.shape==(len1d(y.value),1)
...
_funcs=[
    ...
    (lambda x:(2+x)*(3-x)*(5+x)*(7-x),lambda x:4*x**3-9*x**2-78*x+47),
    (lambda x:(x+2)*(x-3)*(x+5)*(7-x),lambda x:-(4*x**3-9*x**2-78*x+47)),

    # / reciprocal
    (lambda x:2./x,lambda x:-2./x**2),
    (lambda x:x/3,lambda x:1./3.),
    (lambda x:np.reciprocal(x),lambda x:-1./x**2),
    (lambda x:(11+x)*(13-x)/((7+x)*(5-x)),lambda x:4*(x**2+54*x+89)/(x**4+4*x**3-66*x**2-140*x+1225)),
    (lambda x:5*x*np.reciprocal(2+x),lambda x:-5*x/(x + 2)**2 + 5/(x + 2)),
...
def vector_tests():
    for t in generate(_funcs,[np.ones(0),np.ones(1),np.asarray(test_values)],check_vector):
        yield t
